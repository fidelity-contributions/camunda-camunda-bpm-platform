...
public class TelemetryRegistryCounterTest {
  ...
  protected void resetTelemetryCounter() {
    TelemetryRegistry telemetryRegistry = configuration.getTelemetryRegistry();
    Map<String, CommandCounter> entries = telemetryRegistry.getEntries();
    for (String commandName : entries.keySet()) {
      entries.get(commandName).getAndClear();
    }
  }
  ...
  public void shouldCountCommands() {
    ...
    ProcessEngine processEngine =  new StandaloneInMemProcessEngineConfiguration()
        ...
        .buildProcessEngine();
    ProcessEngineConfigurationImpl inMemoryConfiguration = (ProcessEngineConfigurationImpl) processEngine.getProcessEngineConfiguration();
    TelemetryRegistry telemetryRegistry = inMemoryConfiguration.getTelemetryRegistry();
  ...
  public void shouldCountAfterCleaning() {
    ...
    Map<String, CommandCounter> entries = configuration.getTelemetryRegistry().getEntries();
    ...
    assertThat(entries.get("DeployCmd").get()).isEqualTo(0);
  }
}
